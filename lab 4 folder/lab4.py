...
class strumok:
    ...
    def KeyStreamGenerator(key: str, length: int) -> bytes:
        #у цій функції ключ перетворюється у потік байтів за певним алгоритмом. я використаю генератор псевдовипадкових чисел з сідом, але оригінальний шифр значно складніший.
